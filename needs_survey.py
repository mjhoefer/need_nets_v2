...
# FIXING BUG IN first DATA COLLECTION:

# should never be needed again

for i, row in enumerate(selected_needs.iterrows()):
    needs[i]['name'] = row[1]['Need']
    needs[i]['category'] = row[1]['Category']



print("now begin with s=section name. Press q at any time to quit without saving. Press n to save and start the next one.")
curr_input = input('>')

curr_section = ''
skipped = False
count = 0
while curr_input != 'q' and len(files_to_complete) > 0:
    maths = curr_input.split("*")

    if curr_input == 'w' or curr_input == 'W':
        with open(pid + '_TRASH_ME.json', "w") as text_file:
            text_file.write("BAD PDF")
        curr_input = 'n'
        skipped = True
        

    if curr_input == 'n': # next
        if not skipped:
            y = json.dumps(cv, sort_keys=False, indent=4)
            with open(pid + '_complete_gold.json', "w") as text_file:
                text_file.write(y)
            end_time=time.time()
            print (pid, ' took ', end_time-start_time, " seconds to finish. You are beautiful and alive.")
            write_to_log(pid, end_time-start_time, count)
        else:
            print("That PDF will be trashed.")
            skipped = False
        files_to_complete.remove(pid)
        if len(files_to_complete) > 0:
            count = 0
            cv = {}
            pid = files_to_complete[0]
            print("Press enter to start this PID (or type q to quit)")
            print(pid, 'name:', name_dict[pid]['name'], ' at ', name_dict[pid]['university'], ' in department of ', name_dict[pid]['department'])
            print("Press 'w' anytime if something ain't write.")
            quitting = input('>' + pid + ":")
            if quitting == 'q' or quitting == 'Q':
                print("We thank you for your service. Until next time.")
                exit()
            start_time = time.time()
            subprocess.call(['open', pid + '.pdf'])
        else:
            print("YOU'RE DONE!")
            exit()
    elif curr_input.startswith("s="):
        curr_section = curr_input[2:]
    elif len(maths) > 1:
        year = int(maths[0])
        if len(str(year)) == 4:      
            if is_number_like(maths[1]):
                mult = int(maths[1])
                update_dict(curr_section, year, mult)
            else:
                print("Hmmm looks like a multiplication error. Try again.")
    elif len(curr_input) == 4 and is_number_like(curr_input):
        curr_year = int(curr_input)
        if curr_year > 2020 or curr_year < 1920:
            print("That date is after 2020 or before 1920. Try again.")
        else:
            update_dict(curr_section, curr_input, 1)
    else:
        print("did you mean to enter a section? Use s=")
